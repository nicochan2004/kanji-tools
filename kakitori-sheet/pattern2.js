// パターン②: なぞり(お手本)あり
// パターン①と全く同じ仕組み: 単語(複数文字の熟語も可)を入力すると、全単語をつなげた
// 1つの文字列にし、固定サイズのグリッド(横cols×縦rows)の右上の列から、上→下、
// 列は右→左の順(縦書きの読み順)に敷き詰める。文字が足りない残りのマスは空欄になる。
...
(() => {
  ...
  const rowsEl = document.getElementById("p2Rows");
  const addBtn = document.getElementById("p2AddBtn");
  let slotCount = 0;
  let previewEl = null;
  let onChange = null;
  ...
  function buildMasu(charInfo) {
    ...
    if (charInfo) {
      ...
      KD.renderModelChar(masu, charInfo.char, { small: false });
      if (charInfo.isLastOfWord) masu.classList.add("p2-cell-strong");
    }
    return masu;
  }
  ...
  function buildYomiCell(charInfo) {
    ...
    cell.className = "p2-yomi-cell";
    if (charInfo) {
      cell.appendChild(KD.buildVerticalText(charInfo.reading));
      if (charInfo.isLastOfWord) cell.classList.add("p2-cell-strong");
    }
    return cell;
  }
  ...
  function buildPage(pageChars, pageIndex, totalPages, cols, rowsCount) {
    ...
    title.textContent = `なぞり かんじ れんしゅう（${pageIndex + 1}/${totalPages}）`;
